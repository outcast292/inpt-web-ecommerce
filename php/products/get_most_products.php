...
if (!isset($_GET["id_categorie"])) {
    $query = 'SELECT  pr.id_produit,label,prix_produit,count(qtt.id_produit) as qtt_sold from produit pr left join qt_commande qtt on pr.id_produit=qtt.id_produit  where pr.act=1  group by pr.id_produit   order by qtt_sold  desc limit 6 ';
